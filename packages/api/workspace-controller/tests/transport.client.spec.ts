...
describe('WorkspaceController', () => {
  ...
  it('maps generated business failures to the command facade errors', async ({ mock, start }) => {
    ...
    mock.remote.workspace.unarchiveSession.mockResolvedValueOnce(err(new RemoteError('gateway/internal', 'storage failed', {})))
    await expect(controller.unarchiveSession(sid('session')))
      .rejects.toThrow('workspace session unarchive failed: gateway/internal: storage failed')
